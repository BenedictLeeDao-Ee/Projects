...
function Swap(props) {
  ...
  async function fetchDexSwap() {
    ...
    const allowance = await axios.get(`/api/swap/v6.0/1/approve/allowance`, {
      headers,
      params: {
    ...
    if (allowance.data.allowance === "0") {
      // If not approved, get the approve transaction details
      const approve = await axios.get(`/api/swap/v6.0/1/approve/transaction`, {
        headers,
        params: {
          tokenAddress: tokenOne.address,
          amount: "1", // Assuming the amount is 1; adjust accordingly
        },
      });
    ...
    // If allowance is sufficient, proceed with the swap
    const tx = await axios.get(
      `/api/swap/v6.0/1/swap`, {
        headers,
        params: {
    ...
    // Calculate the token amount after the swap
    let decimals = Number(`1E${tokenTwo.decimals}`);
    setTokenTwoAmount((Number(tx.data.toTokenAmount) / decimals).toFixed(2));
